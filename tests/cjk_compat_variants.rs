...
fn test_cjk_compat_variants() {
    ...
    assert_eq!(nfd_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(nfd_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(nfd_iter.next(), None);

    let mut nfkd_iter = s.chars().nfkd();
    assert_eq!(nfkd_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(nfkd_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(nfkd_iter.next(), None);

    let mut nfc_iter = s.chars().nfc();
    assert_eq!(nfc_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(nfc_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(nfc_iter.next(), None);

    let mut nfkc_iter = s.chars().nfkc();
    assert_eq!(nfkc_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(nfkc_iter.next(), Some(Ok('\u{6148}')));
    ...
    assert_eq!(var_nfc_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(var_nfc_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfc_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(var_nfc_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfc_iter.next(), None);

    let mut var_nfd_iter = s.chars().cjk_compat_variants().nfd();
    assert_eq!(var_nfd_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(var_nfd_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfd_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(var_nfd_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfd_iter.next(), None);

    let mut var_nfkc_iter = s.chars().cjk_compat_variants().nfkc();
    assert_eq!(var_nfkc_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(var_nfkc_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfkc_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(var_nfkc_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfkc_iter.next(), None);

    let mut var_nfkd_iter = s.chars().cjk_compat_variants().nfkd();
    assert_eq!(var_nfkd_iter.next(), Some(Ok('\u{831d}')));
    assert_eq!(var_nfkd_iter.next(), Some(Ok('\u{fe00}')));
    assert_eq!(var_nfkd_iter.next(), Some(Ok('\u{6148}')));
    assert_eq!(var_nfkd_iter.next(), Some(Ok('\u{fe00}')));
